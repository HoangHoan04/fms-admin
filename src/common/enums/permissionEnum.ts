...
export const PERMISSION_ACTION_OPTIONS = Object.values(PERMISSION_ACTIONS).map((action) => ({
  code: action.code,
  name: action.name,
}));

export const createPermissionCode = (module: string, action: string): string => {
  return `${module.toUpperCase()}:${action.toUpperCase()}`;
};

export const parsePermissionCode = (code: string): { module: string; action: string } | null => {
