...
export const buildRouteTree = (
  ...
  const traverse = (obj: RouteConfig, parentPath = "", parentKeyVal?: string) => {
...
export const findRouteByKey = (routes: RouteConfig, key: string): any | null => {
...
export const matchPathToPattern = (pathname: string, pattern: string): boolean => {
...
export const findMatchingRoutePattern = (pathname: string, routes: RouteConfig): any | null => {
  ...
  const traverse = (obj: RouteConfig, depth: number, parentKey?: string) => {
    Object.values(obj).forEach((item: any) => {
      if (item.path) {
        ...
        if (isMatch) {
          const hasNoChildren = !item.children || Object.keys(item.children).length === 0;
...
export const convertRoutesToMenuItems = (
  ...
  Object.values(routes).forEach((item: any) => {
    ...
    if (item.key && isShow && canAccess) {
      const menuItem: any = {
        key: item.key,
        label: translateFn && item.translationKey ? translateFn(item.translationKey) : item.label,
        icon: item.icon,
        isShow: item.isShow,
      ...
      if (item.children && Object.keys(item.children).length > 0 && currentDepth < 3) {
...
export const getVisibleRoutes = (routes: RouteConfig): RouteNode[] => {
  ...
  const traverse = (obj: RouteConfig, depth: number = 1, parentKey?: string) => {
