...
const BackToTop = () => {
  ...
  useEffect(() => {
    const findScrollElement = () => {
      const baseViewScroll = document.querySelector(".base-view .overflow-y-auto");
      if (baseViewScroll) return baseViewScroll;
      const tabPanels = document.querySelector(".base-view-tabs .p-tabview-panels");
  ...
  return (
    <>
      {isVisible && (
        <button
          onClick={scrollToTop}
          className="fixed right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-blue-400 p-3.5 text-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-blue-500"
          aria-label="Back to top"
          style={{ bottom: `${position}px` }}
        >
          <i className="pi pi-arrow-up" style={{ fontSize: 18, fontWeight: 800 }}></i>
        </button>
      )}
    </>
  );
