...
export interface FilterField {
  ...
  type:
    ...
    | "custom";
  placeholder?: string;
  options?: { code?: string; label?: string; name?: string; value: any }[];
  onClick?: () => void;
  buttonText?: string;
  disabled?: boolean;
  col?: number;
...
function FilterComponent({
  ...
  const [activeIndex, setActiveIndex] = useState<number | null>(isOpen ? 0 : null);
  ...
  const renderField = (field: FilterField) => {
    ...
    switch (field.type) {
      ...
      case "date":
        return (
          <Calendar
            ...
            className="w-full"
            onChange={(e) =>
              handleFilterChange(field.key, e.value ? dayjs(e.value).format("YYYY-MM-DD") : null)
            }
          />
        );
      case "dateRange":
            ...
            onChange={(e) =>
              handleFilterChange(
                ...
                e.value && e.value[0] && e.value[1]
                  ? [dayjs(e.value[0]).format("YYYY-MM-DD"), dayjs(e.value[1]).format("YYYY-MM-DD")]
                  : null,
              )
            }
          />
      ...
      case "custom":
        return field.render ? field.render(value, (v) => handleFilterChange(field.key, v)) : null;
      default:
        return null;
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="m-2 grid grid-cols-24 gap-4">
          {fields.map((field) => {
            const colClass = {
              3: "lg:col-span-3",
              4: "lg:col-span-4",
            ...
            return (
              ...
              >
                <label className="mb-2 text-xs font-medium">{field.label}</label>

                <div className="flex flex-1 items-center">{renderField(field)}</div>
              </div>
            );
          })}
        </div>
