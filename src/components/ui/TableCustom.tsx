...
export interface TableColumn<T = any> {
  ...
  filterMatchMode?: "startsWith" | "contains" | "endsWith" | "equals" | "notEquals";
...
function TableCustom<T extends Record<string, any>>({
  ...
  const formatters = {
    date: (value: any, format?: string) => {
      ...
      return format === "date" ? date.toLocaleDateString("vi-VN") : date.toLocaleString("vi-VN");
    },
    currency: (value: any, symbol: string = "₫") =>
      value == null ? "-" : `${Number(value).toLocaleString("vi-VN")} ${symbol}`,
    ...
    badge: (
      value: any,
      getSeverity?: (value: any) => "success" | "info" | "warning" | "danger" | "secondary",
    ) => {
      if (!value) return "-";
  ...
  const renderColumnBody = (col: TableColumn<T>) => (rowData: T, options: ColumnBodyOptions) => {
    if (col.body) return col.body(rowData, options);
    if (col.render) return col.render(rowData);
    const value = rowData[col.field];
    if (value == null || value === "") return col.renderEmpty ? col.renderEmpty() : "-";
    switch (col.type) {
      case "date":
  ...
  const actionBodyTemplate = (rowData: T, options: ColumnBodyOptions) => {
    const visibleActions = rowActions.filter((action) => isActionVisible(action, rowData));
          ...
          return (
            ...
            >
              <Button
                ...
                className="bg-transparent! shadow-none! hover:bg-black/5!"
                style={{
                  width: 30,
                  height: 30,
                  minWidth: 30,
  ...
  const getSelectionProps = () => {
    ...
    if (selectionMode === "single")
      return {
        ...
        onSelectionChange: (e: any) => onSelectionChange?.([e.value].filter(Boolean)),
  ...
  return (
    <div className="bg-transparent p-0 text-sm" style={style}>
      <style>{`
      ...
      `}</style>
      {toolbar?.show && (
        <div className={`mb-2 flex items-center gap-3 p-0 ${getToolbarStyle()}`}>
      ...
      {loading ? (
                ...
                style={{
                  width: (() => {
                    const maxActions = Math.max(
                      ...data.map(
                        (row) => rowActions.filter((action) => isActionVisible(action, row)).length,
                      ),
                      1,
                    );
                    if (maxActions <= 3) {
        ...
        <div className="flex flex-col items-center justify-center gap-4 p-8">
          ...
          <span className="text-base font-medium">{emptyText || "Không có dữ liệu"}</span>
        </div>
      )}
      {pagination && !loading && data.length > 0 && (
        <div className="mt-5 flex w-full bg-inherit px-2">
          <Paginator
            currentPageReportTemplate="Hiển thị {first} - {last} trên {totalRecords} dòng"
            ...
            className="custom-grid-paginator w-full! border-none bg-transparent"
          />
        </div>
      )}
      <Tooltip
