...
import { useUploadSingle } from "@/hooks/upload-file";
import { Button } from "primereact/button";
import { Dialog } from "primereact/dialog";
import { type FileUploadHandlerEvent, FileUpload as PrimeFileUpload } from "primereact/fileupload";
...
const mapInitValueToFileList = (
  ...
  const targetValues =
    mode === "multi" ? values : values.length > 0 ? [values[values.length - 1]] : [];
...
function FileUploadCustom({
  ...
  const onCustomUpload = async (event: FileUploadHandlerEvent) => {
    ...
    try {
      ...
      const response = (await onUpload(formData)) as any;
  ...
  const isImageCheck = (urlOrName?: string) => {
    ...
    return /\.(jpg|jpeg|png|gif|webp|svg|bmp|heic|heif)$/i.test(urlOrName) || type === "image";
  };

  const showUploadBtn = !disabled && (mode === "multi" || fileList.length === 0);
  ...
  return (
    <div className={classNames("flex flex-col gap-2", className)} style={style}>
      ...
      <div className="flex flex-wrap gap-4">
          ...
          >
            <div className="flex h-full w-full items-center justify-center">
              {isImageCheck(item.url || item.name) ? (
                <img
                  src={getCloudinaryPreviewUrl(item.url)}
                  alt={item.name}
                  className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
                />
              ) : (
                <div className="flex flex-col items-center justify-center p-2 text-center">
                  <i className="pi pi-file mb-1 text-3xl"></i>
                  <span className="w-full text-[10px] break-all">{item.name}</span>
                </div>
              )}
            </div>
        ...
        {showUploadBtn && (
          <div className={classNames(boxSizeClass, "group relative")}>
            <PrimeFileUpload
              ref={fileUploadRef}
              mode="basic"
              name="file"
            ...
            <div
              className={classNames(
                ...
                "transition-colors duration-200 group-hover:border-blue-500",
      ...
      {!disabled && showUploadBtn && (
        <div className="text-xs">
          {type === "image" ? "Chỉ chấp nhận tệp hình ảnh" : "Chỉ chấp nhận tệp tài liệu"} • Kích
          thước tối đa: {maxSize}MB
        </div>
      )}
        ...
        {previewImage && (
          <div className="flex h-full w-full items-center justify-center p-4">
            {isImageCheck(previewImage.url) ? (
              <img
                src={getCloudinaryPreviewUrl(previewImage.url)}
                alt={previewImage.name}
                className="max-h-[70vh] max-w-full rounded object-contain shadow-lg"
              />
            ) : (
              <div className="p-10 text-center">
                <i className="pi pi-file mb-4 text-6xl"></i>
                <p className="mb-4 text-xl font-bold">{previewImage.name}</p>
                <Button
                  label="Tải xuống"
                  icon="pi pi-download"
