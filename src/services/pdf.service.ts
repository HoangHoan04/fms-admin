...
class PdfService {
  ...
  private readonly DEFAULT_HEADER_COLOR: [number, number, number] = [31, 78, 121];
    ...
    try {
      ...
      const tableRows: RowInput[] = data.map((item: any) =>
        columns.map((col) => {
          ...
          const displayValue = col.formatter ? col.formatter(value) : this.formatCellValue(value);
          return {
  ...
  async exportHtmlToPdf(elementId: string, options: PdfOptions = {}): Promise<PdfExportResult> {
    const {
      filename = `export_${Date.now()}.pdf`,
      title,
  ...
  async printTablePdf<T>(data: T[], columns: PdfColumn[], options: PdfOptions = {}): Promise<void> {
    const {
      title,
      subtitle,
      orientation = "landscape",
  ...
  async getPdfBlob<T>(data: T[], columns: PdfColumn[], options: PdfOptions = {}): Promise<Blob> {
    const {
      title,
      subtitle,
      orientation = "landscape",
