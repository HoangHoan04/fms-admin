...
export const authService = {
  ...
  getUserInfo: async (): Promise<UserInfoResponseDto> => {
    const response = await rootApiService.post<UserInfoResponseDto>(API_ENDPOINTS.AUTH.ME);
    return response;
  },

  logout: async (): Promise<{ message: string }> => {
    const response = await rootApiService.post<{ message: string }>(API_ENDPOINTS.AUTH.LOGOUT);
    return response;
  },
